...
//--------------------------------------------------------------
void ofApp::setup(){
	kinect.setup(0, 3); //Kinect bot to top mapping
	ofSetFrameRate(60);
	ofBackground(0);
	...
	gui.add(koffset.setup("Kinect offset", 1000, -2000, 2000));
	gui.add(sgmode.setup("StepsGrid mode", 0, 0, 1));
...
void ofApp::draw(){
	...
	spout.sendTexture(steps.output.getTexture(), "Steps Grid"); //Output visuals to Spout (sending to MadMapper)
}
...
void ofApp::keyPressed(int key){
	...
	if (key == 'f') {
		ofImage temp;
		steps.output.getTexture().readToPixels(temp.getPixels());
		ofSaveImage(temp, "PROJECTION_01.png", OF_IMAGE_QUALITY_BEST);
	}
}
